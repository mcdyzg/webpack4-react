...
class fetchPlus {
    ...
    // 创建一个请求块，例如所有课程相关的请求可以指定course，调用的时候(new fetchPlug()).course.getList。
    create(name, reqList) {
    ...
    transConfigToFetch(config,contextName){
        ...
        return (data)=>{
            // 调用发送前回调函数
            let cb = t.beforeSendMiddleWare[contextName] || t.beforeSendMiddleWare.total || null
            ...
            .then(data => {
                // 调用请求发送后的成功回调函数
                let cb = t.afterSendMiddleWare[contextName] || t.afterSendMiddleWare.total || null
                ...
                let cb = t.afterSendMiddleWare[contextName] || t.afterSendMiddleWare.total || null
                // 如果返回的是数组，说明有请求回调处理
                if(isArray(cb) && cb[1] && typeof cb[1] === 'function') {
                    cb[1]({errorMsg:'请求错误'})
                }
    ...
    afterUse(){
        ...
        if(typeof resSuccessCb === 'function' || typeof resFailCb === 'function'){
